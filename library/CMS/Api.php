...
class CMS_Api
{
    ...
    public function createPage ($apiKey, $name, $headline, $description, 
            $content)
    {
        if (! $this->_validateKey($apiKey)) {
            return array(
        ...
        // return the page as an array, which Zend_Rest will convert into the
        // XML response
        return $itemPage->toArray();
    }

    public function updatePage ($apiKey, $id, $name, $headline, $description, 
            $content)
    {
        if (! $this->_validateKey($apiKey)) {
            return array(
